...
void
_CSL_MachineCodePrimitive_NewAdd ( const char * name, uint64 morphismAttributes, int64 objectAttributes, block * callHook, byte * function, int64 functionArg, const char *nameSpace, const char * superNamespace )
{
...
CSL_MachineCodePrimitive_AddWords ( CSL * csl )
{
    ...
    for ( i = 0 ; MachineCodePrimitives [ i ].ccp_Name ; i ++ )
    {
        ...
        callHook = 0 ;
        _CSL_MachineCodePrimitive_NewAdd ( p.ccp_Name, p.ui64_CAttribute, p.ui64_CAttribute2, callHook, p.Function, functionArg, p.NameSpace, p.SuperNamespace ) ;
    }
}
#endif    
