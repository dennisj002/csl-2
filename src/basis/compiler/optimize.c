...
// standard locations are :: arg1=reg=ACC=RAX ; arg2=rm=OREG=RCX
// for 2 arg ops : STACK_1= arg1 : STACK_0 = arg2 
// next on this list is a 'previous'
...
_Compiler_GetWordStackState ( Compiler * compiler, Word * word )
{
        ...
        optInfo->node ; optInfo->node = optInfo->nextNode )
    {
        ...
        else if ( optInfo->wordArg2 )
        {
            ...
            if ( optInfo->wordArg1->W_ObjectAttributes & ( CONSTANT | LITERAL ) ) optInfo->wordArg1_literal = true ;
            break ;
        }
        else // nb! : remember this is a rpn optimizer ; get second arg first when going right to left : arg1 arg2 op 
        {
            ...
            optInfo->rvalue = false ;
            if ( optInfo->wordArg2->W_ObjectAttributes & ( CONSTANT | LITERAL ) ) optInfo->wordArg2_literal = true ;
...
Compiler_SetupArgsToStandardLocations ( Compiler * compiler )
{
    ...
    else if ( ( optInfo->NumberOfArgs == 2 ) && optInfo->wordArg2_literal && optInfo->opWord->W_OpInsnCode == CMP ) Compiler_Optimizer_Cmp_With_Arg2Literal ( compiler ) ;
...
// ( ( optInfo->NumberOfArgs == 2 ) && optInfo->wordArg2_literal && optInfo->opWord->W_OpInsnCode == CMP )

void
Compiler_Optimizer_Cmp_With_Arg2Literal ( Compiler * compiler )
{
    CompileOptimizeInfo * optInfo = compiler->OptInfo ;
...
Compiler_Optimizer_2Args_Or_WordArg1_Op ( Compiler * compiler )
{
    ...
    else
    {
        ...
        {
            optInfo->Optimize_Reg = ( ( optInfo->wordArg1->W_ObjectAttributes & REGISTER_VARIABLE ) ? optInfo->wordArg1->RegToUse : ACC ) | REG_ON_BIT ;
            if ( optInfo->wordArg1->W_ObjectAttributes & REGISTER_VARIABLE )
...
Compiler_Optimizer_1Arg ( Compiler * compiler )
{
    ...
    else if ( optInfo->wordArg2->StackPushRegisterCode ) _Set_To_Here_Word_StackPushRegisterCode ( optInfo->wordArg2, 0 ), optInfo->Optimize_Reg = ACC | REG_ON_BIT ;
...
Compiler_Optimizer_WordArg2Op_Or_xBetweenArg1AndArg2 ( Compiler * compiler )
{
        ...
        ( optInfo->xBetweenArg1AndArg2 && optInfo->xBetweenArg1AndArg2->StackPushRegisterCode ) )
    {
        ...
        {
            if ( ! ( ( optInfo->opWord->W_MorphismAttributes & ( CATEGORY_OP_EQUAL ) ) && Compile_Optimize_EqualCheck ( compiler ) ) )
            {
        ...
        if ( ( optInfo->NumberOfArgs == 1 ) && optInfo->wordArg2_Op )
        {
            ...
            if ( word )
            {
                ...
                if ( word->StackPushRegisterCode )
                {
                    ...
                    BI_Block_Copy ( 0, word->StackPushRegisterCode, src, Here - src, 1 ) ;
                }
