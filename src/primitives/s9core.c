 ...
 */
#if 0
#include "../include/s9core.h"
#define S9_S9CORE
#include "../include/s9import.h"
#include "../include/s9ext.h"
#endif
#define S9
...
void
s9_get_counters ( s9_counter **nc, s9_counter **cc, s9_counter **vc,
    s9_counter **gc )
{
    *nc = & Nodes ;
    *cc = & Conses ;
...
int
s9_inport_open_p ( void )
{
    return Ports[Input_port] != NULL ;
}

int
s9_outport_open_p ( void )
{
...
s9_readc ( void )
{
    ...
    else
    {
        ...
        return s9_getChar ( Ports[Input_port] ) ; //getc ( Ports[Input_port] ) ;
    }
}
...
s9_rejectc ( int c )
{
    ...
    {
        s9_ungetChar ( c, Ports[Input_port] ) ;
    }
}
...
void
s9_set_node_limit ( int n )
{
    Node_limit = n * 1024L ;
}

void
s9_set_vector_limit ( int n )
{
    Vector_limit = n * 1024L ;
}

void
s9_gc_verbosity ( int n )
{
    Verbose_GC = n ;
}

void
s9_mem_error_handler ( void (*h )( int src ) )
{
    Mem_error_handler = h ;
}

static void
new_cons_segment ( void )
{
...
mark ( cell n )
{
    ...
    while ( 1 )
    {
        if ( s9_special_p ( n ) || ( S9_tag [n] & S9_MARK_TAG ) ) //Tag[n] & S9_MARK_TAG ) )
        {
            if ( parent == NIL )
                break ;
            if ( Tag[parent] & S9_VECTOR_TAG )
            { /* S1 --> S1|done */
            ...
            else if ( Tag[parent] & S9_STATE_TAG )
            { /* S1 --> S2 */
                p = cdr ( parent ) ;
...
cell
s9_cons3 ( cell pcar, cell pcdr, int ptag )
{
    cell n ;
    int k ;
    char buf[100] ;
...
cell
s9_symbol_table ( void )
{
    return Symbols ;
}

cell
s9_symbol_ref ( char *s )
{
    cell y, new ;
...
static cell
S9_make_quick_real ( int flags, cell exp, cell mant )
{
    cell n ;
...
cell
S9_make_real ( int flags, cell exp, cell mant )
{
    cell r ;
...
cell
s9_make_real ( int sign, cell exp, cell mant )
{
    cell m ;
    int i ;
...
cell
s9_make_port ( int portno, cell type )
{
    cell n ;
    int pf ;
...
cell
s9_flat_copy ( cell n, cell *lastp )
{
    cell a, m, last, new ;
...
int
s9_bignum_less_p ( cell a, cell b )
{
...
int
s9_bignum_equal_p ( cell a, cell b )
{
    a = cdr ( a ) ;
    b = cdr ( b ) ;
    while ( a != NIL && b != NIL )
...
cell
s9_bignum_subtract ( cell a, cell b )
{
    Tmp = b ;
    prot ( a ) ;
    prot ( b ) ;
...
cell
s9_bignum_shift_left ( cell a, int fill )
{
    cell r, c, result ;
    int carry ;
...
cell
s9_bignum_multiply ( cell a, cell b )
{
    int neg ;
    cell r, i, result ;
...
cell
s9_bignum_divide ( cell a, cell b )
{
    if ( bignum_zero_p ( b ) )
        return UNDEFINED ;
    Tmp = b ;
...
static cell
scale_mantissa ( cell r, cell desired_e, int max_size )
{
    int dgs ;
    cell n, e ;
...
static int
real_compare ( cell a, cell b, int approx )
{
    cell ma, mb, d, e ;
    int p ;
...
int
s9_real_equal_p ( cell a, cell b )
{
    return real_compare ( a, b, 0 ) ;
}

int
s9_real_approx_p ( cell a, cell b )
{
    return real_compare ( a, b, 1 ) ;
}

int
s9_real_less_p ( cell a, cell b )
{
    cell ma, mb ;
    int ka, kb, neg ;
    int dpa, dpb ;
...
cell
s9_real_subtract ( cell a, cell b )
{
    cell r ;
...
cell
s9_real_multiply ( cell a, cell b )
{
...
static cell
rpower ( cell x, cell y, cell prec )
{
    cell n, nprec ;
...
cell
s9_real_trunc ( cell x )
{
    return rround ( x, 0 ) ;
}

cell
s9_real_floor ( cell x )
{
    return rround ( x, 1 ) ;
}
...
static int
exponent_char_p ( int c )
{
    return c && strchr ( Exponent_chars, c ) != NULL ;
}

int
s9_integer_string_p ( char *s )
{
    if ( *s == '-' || * s == '+' )
        s ++ ;
    if ( ! *s )
...
cell
s9_bignum_to_int ( cell x, int *of )
{
    int a, b, s ;
...
cell
s9_real_to_string ( cell x, int mode )
{
#define Z S9_MANTISSA_SIZE+S9_DIGITS_PER_CELL+10
    ...
    switch ( mode )
    {
        case 0: s9_print_real ( x ) ;
            break ;
        case 1: s9_print_sci_real ( x ) ;
            break ;
        case 2: s9_print_expanded_real ( x ) ;
            break ;
        default:
            Str_outport = NULL ;
            Str_outport_len = 0 ;
...
int
s9_open_output_port ( char *path, int append )
{
    int i = s9_new_port ( ) ;
...
static char *
expected ( int n, cell who, char *what )
{
    static char msg[100] ;
    S9_PRIM *p ;
...
char *
s9_dump_image ( char *path, char *magic )
{
    FILE *f ;
    cell n, **v ;
    int i ;
...
char *
s9_load_image ( char *path, char *magic )
{
    FILE *f ;
    cell n, **v ;
    int i ;
...
void
s9_exponent_chars ( char *s )
{
    Exponent_chars = s ;
}

void
s9_image_vars ( cell **v )
{
    Image_vars = v ;
}

void
s9_add_image_vars ( cell **v )
{
    int i, n, m ;
    cell **nv ;
...
{
    Car = 0, Cdr = 0, Tag = 0 ;
    Vectors = 0, Nullvec = 0, Nullstr = 0, Blank = 0 ;
    ...
    GC_ext_roots = NULL ; //
    Cons_segment_size = Vec_segment_size = 0 ;
    Cons_pool_size = Vec_pool_size = 0 ;
    Verbose_GC = 0 ;
}

void
s9_init ( cell **extroots, cell *stack, int *stkptr )
{
    int i ;
...
void
print_test ( char *name, void (*printer )( cell ), cell n, char *s )
{
    int p, op, i ;
    char b[100] ;
