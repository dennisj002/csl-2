...
cell *GC_roots[] = { &Prog, &Env, &Cts, &Emitbuf, &Glob, &Hash, &Macros, &Rts, &Acc, &Ep, &E0, &Argv, &Tmp, NULL } ;
//cell *GC_roots [14] = { [0] = & Prog, [1] = & Env, [2] = & Cts, [3] = & Emitbuf, [4] = & Glob, [5] = & Hash,
//    [6] = & Macros, [7] = & Rts, [8] = & Acc, [9] = & Ep, [10] = & E0, [11] = & Argv, [12] = & Tmp, [13] = NULL } ;
...
enum
{
    OP_APPLIS, OP_APPLY, OP_ARG, OP_COPY_ARG, OP_CLOSURE,
...
cell
subvector ( cell x, int k0, int k1 )
{
    cell n, *vx, *vn ;
    int i, j ;
...
cell
list_to_vector ( cell m, char *msg, int flags )
{
    cell n, vec ;
    int k ;
    cell *p ;
...
void
add_primitives ( char *name, S9_PRIM *p )
{
    cell x, v, n, b ;
    int i ;
...
meta_command ( void )
{
    ...
    switch ( cmd )
    {
        case 'a': cmdsym = symbol_ref ( "apropos" ) ;
            break ;
        case 'h': cmdsym = symbol_ref ( "help" ) ;
            break ;
        case 'l': cmdsym = symbol_ref ( "load-from-library" ) ;
            break ;
...
cell
bignum_read ( char *pre, int radix )
{
...
void
ckargs ( cell x, char *who, int min, int max )
{
    int k ;
    char buf[100] ;
...
int
ckbegin ( cell x, int top )
{
    return ckseq ( cdr ( x ), top ) ;
}

int
ckdefine ( cell x, int top )
{
    ckargs ( x, "define", 2, 2 ) ;
    if ( ! symbol_p ( cadr ( x ) ) )
...
cell
integer_argument ( char *who, cell x )
{
    cell n ;
    char msg[100] ;
...
int
assqv ( char *who, int v, cell x, cell a )
{
    cell p ;
    char buf[64] ;
...
int
memqv ( char *who, int v, cell x, cell a )
{
    cell p ;
    char buf[64] ;
...
int
nth ( char *who, int ref, cell a, cell n )
{
    cell p ;
    char buf[64] ;
    int k ;
...
cell
substring ( cell s, cell n0, cell n1 )
{
    int k, k0, k1, i, j ;
    cell n ;
    char *s0, *s1 ;
...
cell
vcopy ( cell v, cell n0, cell nn, cell fill )
{
    cell n, *ov, *nv ;
    int k0, kn, k ;
    int i, j ;
...
void
write_obj ( cell x, int p, int disp )
{
    int pp, odisp ;
...
void
keyboard_interrupt ( void *dummy, char *note )
{
